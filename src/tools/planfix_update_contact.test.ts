...
describe('planfix_update_contact tool', () => {
  ...
  it('updates name and splits it into first and last name', async () => {
    ...
    mockPlanfixRequest.mockResolvedValueOnce({ contact: { ...mockContact, name: '', lastname: '' } });
    mockPlanfixRequest.mockResolvedValueOnce({});
  ...
  it('updates telegram username when different', async () => {
    ...
    expect(mockPlanfixRequest).toHaveBeenNthCalledWith(
      ...
      {
        customFieldData: [
          { field: { id: 1001 }, value: '@new_username' }
        ]
      }
    );
  ...
  it('handles dry run mode', async () => {
    ...
    try {
      ...
      vi.doMock('../config.js', () => ({
        ...
        PLANFIX_FIELD_IDS: { telegram: 1001 }
      }));
      ...
      const { updatePlanfixContact } = await import('./planfix_update_contact.js');
      const { planfixRequest: mockPlanfixRequest } = await import('../helpers.js');
