...
describe('planfix_add_to_lead_task tool', () => {
  it('should create a new lead task with contact details', async () => {
    ...
    expect(result.valid, `Tool MCP response was invalid. Raw response: ${JSON.stringify(result.parsed)}`).toBe(true);

    // Check the content returned by the tool itself
    const content = result.content as z.infer<typeof AddToLeadTaskOutputSchema>;
    expect(content.error, `Tool returned an error: ${content.error}`).toBeUndefined();
    ...
    expect(content.url).toContain(`https://expertizeme.planfix.com/task/${content.taskId}`);
    expect(content.clientUrl).toContain(`https://expertizeme.planfix.com/contact/${content.clientId}`);
